...
class App extends React.Component {
    //state variables include type for the current screen (reflected in right panel), previous type, and the index currently being viewed if applicable
    state = {type: 1, prev: 1, current: 0}
    obj = -1;

    //get request to access the necessary information
    request() {
    ...
    //methods for accessing and formatting data attributes
    get(i) {
        return this.obj[i]
    }
    ...
    //determines the display for the right side of the screen
    getRightPanel(i) {
        if (this.state.type === 1) {
            ...
            //display google map with a marker at the current location (due to the API key being free, the marker displays in default location)
            ...
            //display detailed location information and times open
                    ...
                    <div id="overlay">
                        <div id="overlay2">
                            ...
                            <div style={{textAlign: "center"}}>
                                <!-- times open, contains nonbreaking spaces for enforced uniformity -->
                            ...
                            <br/><br/>
                                //button leading to the given URL
                            <div style={{textAlign: "center"}}>
    ...
    //asynchronously renders google maps for state 2
    async statetransition(id){
        const prev = this.state.type;
        //due to potential issues launching google maps from a page it is currently visible, a quick transition is made as necessary
    ...
    //these methods get the id of the panel being clicked, and perform state transitions accordingly
    ...
    //the actual display for the webpage
    render() {
        //allows the "directions" to launch a map that has a destination at the specified latitude and longitude
        const showInMapClicked = (event) => {
            const id = event.target.id;
            window.open("https://www.google.com/maps/dir//"+ this.getLat(id)+","+this.getLong(id));
        };
        //if the data has not yet been accessed, it is as part of the render process
        if (this.obj === -1) {
            this.request()
        }

        //builds the output by adding one panel per data entry in "obj"
        var rows = [];
        var i = 0;
        while (i < this.obj.length) {
            rows.push(
                <div className="Panel">
                    <!-- information and handlers for clicking the panel and buttons -->
        ...
        //final output is the set of panels that have just been defined on the left side, and the right panel based on the current state.type
        return (
            <div>
                <div className="Left">
                    {rows}
